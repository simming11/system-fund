...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await ApiAuthService.registerStudent(
        ...
        yearEntryNumber,
        Course,
        DOB ? DOB.toISOString() : "",  // Ensure fallback for undefined DOB
        gpaNumber,
        Religion
      );
  ...
  <input
    ...
    onChange={(e) => {
      const value = e.target.value;
      // Use regex to allow only alphabetic characters and spaces
      const regex = /^[ก-๙a-zA-Z\s]*$/;
      if (regex.test(value)) {
        setFirstName(value); // Set value if it's valid
        setErrors((prevErrors) => ({
          ...prevErrors,
          FirstName: '', // Clear error if valid
        }));
      } else {
        // Set error if non-alphabetic characters are entered
        setErrors((prevErrors) => ({
          ...prevErrors,
          FirstName: 'กรุณากรอกตัวอักษรเท่านั้น',
        }));
      }
    }}
  ...
  <input
    ...
    onChange={(e) => {
      ...
      const regex = /^[ก-๙a-zA-Z\s]*$/;
      if (regex.test(value)) {
        setLastName(value); // Set value if it's valid
        setErrors((prevErrors) => ({
          ...prevErrors,
          LastName: '', // Clear error if valid
        }));
      } else {
        // Set error if non-alphabetic characters are entered
        setErrors((prevErrors) => ({
          ...prevErrors,
          LastName: 'กรุณากรอกตัวอักษรเท่านั้น',
        }));
      }
    }}
...
<div className="w-full">
  ...
  <input
    type="text" // Using type="text" for easier regex handling
    placeholder="อีเมล"
    value={Email}
    onChange={(e) => {
      const value = e.target.value;

      // Always update the email value first
      setEmail(value);

      // Check for Thai characters
      const containsThaiCharacters = /[ก-๙]/.test(value);

      // Check if email ends with @tsu.ac.th
      const isValidEmail = /^[^\s@]+@tsu\.ac\.th$/.test(value);

      if (containsThaiCharacters) {
        setErrors((prevErrors) => ({
          ...prevErrors,
          Email: "ห้ามใช้อักษรภาษาไทยในอีเมล", // Show error if Thai characters are found
        }));
      } else if (!isValidEmail) {
        setErrors((prevErrors) => ({
          ...prevErrors,
          Email: "กรุณากรอกอีเมลที่ลงท้ายด้วย @tsu.ac.th เท่านั้น", // Show error if invalid email domain
        }));
      } else {
        setErrors((prevErrors) => ({
          ...prevErrors,
          Email: "", // Clear errors if valid
        }));
      }
    }}
